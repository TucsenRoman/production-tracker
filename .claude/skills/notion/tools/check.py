...
# A focus ring is a ring, not elevation. It is the one shadow a resting element may carry.
FOCUS_HINT = re.compile(r":focus", re.I)
...
def is_neutralish(r, g, b):
    """Neutral by channel spread, threshold 12.

    Saturation looks like the right metric and is not: at the lightness these
    surfaces live at, the HSL denominator collapses and #f9f8f7 (a warm grey,
    spread 2) scores more saturated than #dbeddb (a green tint, spread 18).
    A flat spread threshold of 12 separates the two families correctly at every
    lightness — greys below it, tints above.
    """
    return max(r, g, b) - min(r, g, b) <= 12


def check(text, name):
    # Comments discuss the bans ("Ink. Never #000.") — analysing them reports the
    # documentation as a violation. Strip them before anything else looks at the text.
    ...
    # A utility-class codebase declares almost no font-size in CSS; judging the
    # scale off one or two declarations cries wolf. Below 3, this is a MANUAL check.
    if len(sizes) >= 3:
        if len(sizes) > 5:
    ...
    for m in re.finditer(r"([^{}]+)\{([^}]*box-shadow\s*:\s*(?!none)[^;}]+)", text):
        ...
        if OVERLAY_HINT.search(sel) or FOCUS_HINT.search(sel):
            continue
