 ...
 * in, whether Clover is answering, and who has approved what on it.
 *
 * It exists because the tablet's top strip is gone. That strip carried the
 * shop name and a "Change shop" chevron that, with one location configured,
 * never even rendered — so the one setting a floor tablet has was reachable
 * only in a demo state nobody runs. Here it is a tab.
 *
 * The approval log is the reason this screen is worth its place in the bar.
 * `useApprovalLog` has been recording every gated action since approvals were
 * built — who, what, when, which shop — and nothing has ever rendered it. On
 * a floor with no sign-in and no crew accounts that log IS the audit trail,
 * and an audit trail nobody can read is a file, not a record.
 *
 * Styled to the floor task-list DNA (.claude/skills/floor-task-list): ruled
 * sections, indented rows, hairlines not cards, one primary action.
 */
...
/** Matches the Inventory toolbar's old cooldown, so a double-tap on the floor
 *  doesn't hammer Clover. */
...
function CloverRow({ status, syncedAt, onRefresh }) {
  ...
  /* Status carries a colour, because "is the till talking to us" is the one
   * thing on this screen somebody checks from a step away. Green connected,
   * red unreachable, plain grey while a sync is in flight — a spinner is
   * already saying "working", so tinting it too would spend a colour on a
   * state that resolves itself in a second. */
...
export default function SettingsScreen({
  ...
  /* Capped at the most recent 40. The store keeps 200; a floor terminal
   * scrolling a year of approvals is a different screen than this one. */
