...
/** A schedule entry with the station it was planned at folded in: the caller
 *  flattens the day's stations into one list before handing it over. */
export type ScheduledEntry = ScheduleEntry & { station: StationName };

/** Traffic light on the floor count: nothing / below par / at or over par. */
const FLOOR_TONE: Record<StockStatus, string> = {
  out: "text-danger",
  low: "text-warn",
  ok: "text-ok",
};

/** What `onSave` commits. Only the editable half of the record. */
...
export interface ItemModalProps {
  /** Normalized — see `normalizeItem` in lib/domain. */
  item: InventoryItem;
  /** Recent selling pace, for the cover figures. No velocity, no number. */
  perDay?: number | null;
  /** Optional context, pre-filtered to this product. */
  activeBatches?: Batch[];
  scheduledToday?: ScheduledEntry[];
  recentBatches?: ClosedBatch[];
  onClose: () => void;
  onSave: (product: string, patch: ItemPatch) => void;
...
export default function ItemModal({
  ...
  const [type, setType] = useState<ProductFamily>(family);
