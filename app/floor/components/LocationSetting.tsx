...
import type { CompanyLocation, CompanyPerson } from "../../company/lib/types";
...
export function LocationPicker({ locations, onPick }: LocationPickerProps) {
  return (
 ...
 */
export interface LocationSwitchDialogProps {
  current?: CompanyLocation | null;
  locations: CompanyLocation[];
  onCancel: () => void;
  /** The person who approved the move, for the audit trail. */
  onSwitch: (id: string, by: CompanyPerson) => void;
}

export function LocationSwitchDialog({
  current,
  locations,
  onCancel,
  onSwitch,
}: LocationSwitchDialogProps) {
  ...
  const approve = () => {
    ...
    if (picked) onSwitch(picked.id, person);
  };
