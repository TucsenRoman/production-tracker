...
/** One row of `/api/inventory`, i.e. what Clover speaks for. */
...
/** Whether the catalogue on screen came from Clover. */
type CloverStatus = "loading" | "live" | "error";

/** What the product editor commits. Wider than ItemModal's own patch: the
 *  band editor can also move `minBatch`. */
type ProductPatch = Partial<StockBand> & {
  type?: ProductFamily;
  unit?: string;
};

/** What a screen hands over for a NEW task — the rest is stamped here. */
type NewTaskFields = Omit<
  Task,
  "id" | "completed" | "createdBy" | "createdAt"
>;
...
function Application() {
  ...
  const [view, setView] = useState<string>("batches");
  ...
  const [cloverStatus, setCloverStatus] = useState<CloverStatus>("loading");
  const [syncedAt, setSyncedAt] = useState<string | null>(null);
  /** product name → units sold per day, from /api/sales. */
  ...
  const loadClover = useCallback(async () => {
    ...
    try {
      ...
      const items: CloverItem[] = (data.items || []).filter(
        (i: CloverItem) => !i.hidden,
      );
   ...
   */
  const addStock = (product: string, amount: number, state: StockState) => {
    setInventory((prev) => {
      ...
      if (!known) {
        return [
          ...
          normalizeItem({
            ...
            freezer: 0,
            floor: 0,
            ...({ [state]: amount } as Partial<StockLevels>),
      ...
      return prev.map((i) => {
        ...
        patch[state] = +(item[state] + amount).toFixed(1);
        return { ...item, ...patch };
      });
    });
  };
  ...
  const handleUpdateProduct = (product: string, patch: ProductPatch) => {
  ...
  const handleAddProduct = (item: StoredInventoryItem) => {
  ...
  const handlePutOut = (product: string) => {
    setInventory((prev) =>
  ...
  const handleWeighIn = async (id: string, boxWeight: number) => {
    const batch = batches.find((b) => b.id === id);
    const by = await approve("weigh-in", { detail: batch && `${batch.id} ${batch.product}` });
    if (!by) return;
    setBatches((prev) =>
  ...
  const handleCreateTask = (task: NewTaskFields, staff: { name: string }) => {
    setTasks((prev) => [
      {
        id: newId("TD"),
        completed: false,
  ...
  const handleToggleTask = (id: string, staff: { name: string }) => {
    setTasks((prev) =>
      prev.map((t) => {
        if (t.id !== id) return t;
  ...
  const handleRenameTaskCategory = (id: string, label: string) => {
    setTaskCategories((prev) => prev.map((c) => (c.id === id ? { ...c, label } : c)));
  };

  const handleRemoveTaskCategory = (id: string) => {
   ...
   */
  const handleStartPlanned = (station: StationName, entryId: string) => {
  ...
  const handleRemovePlanned = (station: StationName, entryId: string) => {
    setSchedule((prev) => ({
      ...prev,
      [today]: {
        ...prev[today],
    ...
    >
      ...
      {current === "tasks" && (
        <TasksScreen
          ...
          onToggle={(id: string) => handleToggleTask(id, user)}
