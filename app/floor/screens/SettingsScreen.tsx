...
import type { ApprovalLogEntry } from "../lib/approval";
import type { CompanyLocation } from "../../company/lib/types";

/** Mirrors the tracker's own `CloverStatus`. */
export type CloverStatus = "loading" | "live" | "error";
...
function ApprovalRow({ entry }: { entry: ApprovalLogEntry }) {
  return (
...
export interface SettingsScreenProps {
  /** The shop this tablet is standing in. */
  place?: CompanyLocation | null;
  locations?: CompanyLocation[];
  /** Newest first. The floor has no sign-in, so this IS the audit trail. */
  log?: ApprovalLogEntry[];
  cloverStatus: CloverStatus;
  syncedAt?: string | null;
  onRefresh: () => void;
  /** Omitted when there is nothing to switch to. */
  onChangeShop?: (() => void) | null;
}
