 ...
 */
/** Today's yield readout, as the ticker renders it. */
interface YieldStats {
  byStation: Record<StationName, number>;
  complete: number;
  ...
  yieldPrevDay: string | null;
}

/** A plan line with the station it sits at folded in. */
type PlannedEntry = ScheduleEntry & { station: StationName };

/** What MoveDialog hands back when the operator confirms. */
...
function YieldTicker({ stats }: { stats: YieldStats }) {
...
function PlannedToday({
  ...
  batches: Batch[];
  showStation?: boolean;
  onStart: (station: StationName, entryId: string) => void;
  onRemove: (station: StationName, entryId: string) => void;
}) {
...
function QuickAddToStation({
  ...
  station: StationName;
  products: string[];
  onAdd: (station: StationName, product: string, qty: number) => void;
}) {
...
function MoveDialog({
  ...
  /** null while closed — the dialog stays mounted so its state survives. */
  batch: Batch | null;
  onCancel: () => void;
  onCommit: (commit: MoveCommit) => void;
}) {
  ...
  const willFinalize = stage != null && isFinalStage(stage);
  ...
  const [destination, setDestination] = useState<StockState>("made");
...
export interface BatchesScreenProps {
  ...
  history?: ClosedBatch[];
  today: string;
  onWeighIn: (id: string, boxWeight: number) => void;
  onFinalize: (id: string, finalWeight: number, destination: StockState) => void;
  onAdvance: (id: string) => void;
  onQueueForToday: (station: StationName, product: string, qty: number) => void;
  onStartPlanned: (station: StationName, entryId: string) => void;
  onRemovePlanned: (station: StationName, entryId: string) => void;
}
...
export default function BatchesScreen({
  ...
  const stats = useMemo(() => {
    const at = (s: StationName) =>
      batches.filter((b) => b.stage === s).length;
  ...
  const commit = (payload: MoveCommit) => {
    const { batch, weight, destination, willWeighIn, willFinalize } = payload;
    if (willFinalize && destination) onFinalize(batch.id, weight, destination);
