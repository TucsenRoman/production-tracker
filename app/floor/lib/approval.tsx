...
import type { CompanyRoster } from "./companyRoster";
...
import type {
  ...
  GatedAction,
  Permissions,
} from "../../company/lib/types";

/** What an approval resolves to: the person who stood behind it, the
 *  ungated sentinel, or null when the dialog was cancelled. */
export type Approver = CompanyPerson | UngatedApproval | null;

export interface UngatedApproval {
  id: null;
  name: null;
  ungated: true;
}
...
/** One line of the tablet's audit trail. */
...
const ApprovalContext = createContext<ApproveFn | null>(null);

export function useApproval(): ApproveFn {
...
export function useApprovalLog() {
  return usePersistentState<ApprovalLogEntry[]>("approvalLog", []);
}

/** Resolved for an action nobody had to approve. */
const NO_APPROVAL_NEEDED: UngatedApproval = { id: null, name: null, ungated: true };
...
export function ApprovalProvider({ location, children }: ApprovalProviderProps) {
  const [permissions] = useCompanyState<Permissions>(
    "permissions",
    defaultPermissions(),
  );
  ...
  const resolveRef = useRef<((value: Approver) => void) | null>(null);

  const approve = useCallback<ApproveFn>(
    (actionId, { detail } = {}) => {
      const action = GATED_ACTIONS.find(
        (a: GatedAction) => a.id === actionId,
      ) as GatedAction | undefined;
  ...
  const finish = (person: CompanyPerson | null) => {
    setRequest(null);
...
interface ApprovalDialogProps {
  ...
  allows: (actionId: string, userId: string) => boolean;
  /** How many people are named, for a targeted action; null otherwise. */
  namedCount: number | null;
  onCancel: () => void;
  onApprove: (person: CompanyPerson) => void;
}
