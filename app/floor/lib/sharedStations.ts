...
import type { StationConfig, StationName } from "../../lib/domain";

/** The live station list; falls back to the shipped default if the console
 *  has stored an empty one. */
export function useSharedStations(): StationName[] {
  const [stations] = useCompanyState<StationName[]>(
    "stations",
    COMPANY_SEED.stations,
  );
...
export function useSharedStationConfig(): Record<StationName, StationConfig> {
  const [config] = useCompanyState<Record<StationName, StationConfig>>(
    "stationConfig",
    {},
  );
