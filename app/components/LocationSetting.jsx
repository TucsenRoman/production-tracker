...
/**
 * The two faces of "which shop is this tablet": the one-time question, and
 * the guarded way to change the answer later.
 *
 * Both live here rather than in a screen because neither belongs to a screen —
 * the picker gates the whole app, and the switch hangs off the shell.
 */

import React, { useState } from "react";
import { Check, MapPin, ShieldCheck } from "lucide-react";

import { Button, Modal, PinInput, cx } from "./ui";
 ...
 * protect yet, and a terminal that demands a PIN before it will admit which
 * building it is in is a locked door in front of an empty room.
 */
...
/**
 * Changing it afterwards. Two steps on purpose: pick the shop, then prove you
 * are someone the Permissions screen named for `switch-location`.
 *
 * The PIN here is doing the job it was defined to do — approving one action,
 * not signing anyone in. Nothing about the tablet's session changes; the
 * person punching it walks away again.
 */
export function LocationSwitchDialog({ current, locations, onCancel, onSwitch }) {
  ...
  const approve = () => {
    ...
    /* Named, not merely senior. An admin who was left off the list is still
     * refused — otherwise the assignment on the Permissions screen would be
     * decoration over a role check. */
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {/* The warning belongs next to the thing it warns about, and only once
         *  a change is actually on the table. */}
        {changing && (
