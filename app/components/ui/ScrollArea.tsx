...
export interface ScrollAreaProps extends HTMLAttributes<HTMLElement> {
  /** Every caller uses the default; see the note at `Tag` below. */
  as?: ElementType;
  axis?: "x" | "y";
  /** "auto" arms only once the content outgrows the box; `false` leaves the
   *  overflow property to the caller (AppShell's rail mode turns it on only
   *  at `lg:`). */
  arm?: boolean | "auto";
  /** px held open across the grain for a decoration that pokes outside a
   *  child's box (a corner badge). `axis="x"` only, and deliberately. */
  clipRoom?: number;
  /** Fade whichever edge still has content behind it. */
  fade?: boolean;
  /** Width of that fade in px. 16 was invisible; 40 reads. */
  band?: number;
  /** Hold open enough slack at the far end that the scroll range always
   *  reaches the resting point. Measured, not a constant. */
  void?: boolean;
  /** Glide a tapped child toward the middle. */
...
export function ScrollArea({
  ...
  /* `as` is public, but no caller has ever passed one, and typing a
   * polymorphic component that also forwards a ref costs more machinery than
   * the prop earns. Narrowed to the default so the props below are checked
   * against something real; any other tag still renders at runtime. */
  const Tag = as as "div";
  ...
  const measure = useCallback(() => {
    ...
    while (
      last &&
      (last.hasAttribute("data-rail-spacer") ||
        last.hasAttribute("data-scroll-void"))
    )
    ...
    const voidEl = el.querySelector<HTMLElement>(":scope > [data-scroll-void]");
    const appliedSlack = voidEl
      ? Math.round(
          voidEl.getBoundingClientRect()[vertical ? "height" : "width"],
        )
      : 0;
    ...
    const contentExtent = last
      ...
      : el[A.size] -
        (parseFloat(getComputedStyle(el)[A.padEnd]) || 0) -
        appliedSlack;
    ...
    if (voidEnd) {
      ...
      const scrolls = /auto|scroll/.test(
        vertical ? cs.overflowY : cs.overflowX,
      );
      const anchor =
        el.querySelector<HTMLElement>("[data-scroll-anchor]") ||
        el.querySelector<HTMLElement>("[data-screen-toolbar]");
      if (scrolls && anchor) {
        ...
        if (restRef.current != null) {
          ...
          want = Math.max(
            0,
            Math.round(appliedSlack + restRef.current - range),
          );
        }
      }
    }
  ...
  const crossRoom: CSSProperties | null =
    armed && room ? { paddingTop: room, marginTop: -room } : null;

  const center = (e: ReactMouseEvent<HTMLElement>) => {
    const el = railRef.current;
    ...
    const to = start + size / 2 - el[A.client] / 2;
