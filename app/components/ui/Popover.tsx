...
export interface PopoverProps {
  /** Controlled on purpose — see below. */
  open: boolean;
  onClose?: () => void;
  content?: ReactNode;
  side?: "top" | "bottom";
...
export function Popover({
  ...
  useLayoutEffect(() => {
    ...
    const place = () => {
      ...
      const placed = room[side] ? side : room[opposite] ? opposite : side;
  ...
  useEffect(() => {
    ...
    const onDown = (e: Event) => {
      ...
      if (target && wrapRef.current?.contains(target)) return; // the trigger's own toggle
      if (target && panelRef.current?.contains(target)) return; // working inside the panel
      onClose?.();
    };
