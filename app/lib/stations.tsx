...
import type { Batch, StationConfig, StationName } from "./domain";

/** A batch as it may arrive from storage: `stage` was an INDEX in an older
 *  build. See `migrateStage`. */
export type StoredBatch = Omit<Batch, "stage"> & { stage: string | number };

export interface StationIconChoice {
  key: string;
  label: string;
  Icon: LucideIcon;
}

export interface StationIconGroup {
  label: string;
  icons: StationIconChoice[];
}

/** What `useStations()` hands back. */
export interface StationsApi {
  ...
  lastProductionStage: StationName | null;
  isFinalStage: (stage: string) => boolean;
  finalStage: string;
  nextStage: (batch: Pick<Batch, "stage" | "needsSmoke">) => string;
  normalizeStage: (batch: StoredBatch) => Batch;
}

const KNOWN_ICON: Record<string, LucideIcon> = {
  Smokehouse: Flame,
  Packaging: Package,
 ...
 */
export const STATION_ICON_GROUPS: readonly StationIconGroup[] = [
  {
    label: "Prep",
    icons: [
...
const STATION_ICON_CHOICES: StationIconChoice[] = STATION_ICON_GROUPS.flatMap(
  (g) => g.icons,
);
...
const RETIRED_ICON: Record<string, LucideIcon> = {
  boxes: Package,
  container: Package,
  vault: Package,
...
export function migrateStage(
  ...
  if (typeof batch?.stage !== "number") return batch as Batch;
  return {
    ...batch,
    stage: stages[Math.min(batch.stage, stages.length - 1)] ?? stages[0],
  };
}

const StationsContext = createContext<StationsApi | null>(null);
...
export interface StationsProviderProps {
  stations?: StationName[];
  config?: Record<StationName, StationConfig>;
...
export function StationsProvider({
  ...
  const value = useMemo<StationsApi>(() => {
    ...
    return {
      ...
      iconFor: (name) => {
        const chosen = config[name]?.icon;
        return (
          (chosen ? ICON_BY_KEY[chosen] : undefined) ||
          KNOWN_ICON[name] ||
          Factory
        );
      },
