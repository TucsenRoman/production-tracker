...
/**
 * Manager approval for a gated action.
 *
 * This is the piece the Permissions screen has been configuring since it was
 * built, and which nothing on the floor has ever enforced: no screen read
 * `permissions`, no code path asked for a lead PIN. The rules existed, the
 * gate did not.
 *
 * The shape it takes now follows from what the tablet became. There is no
 * sign-in and no session — the iPad's own passcode is the lock, ProTrack
 * opens straight up, and everything on the floor is visible because none of
 * it is worth hiding. So a PIN is never a way IN. It appears at exactly the
 * moments an admin decided somebody should have to stand behind a decision,
 * and then the person walks away again.
 *
 ...
 *   commit({ approvedBy: by });  // `by` is null-safe: ungated actions resolve
 *                                // to a sentinel, not a person
 *
 * Ungated actions resolve immediately without a dialog, so a caller can wrap
 * every action the same way and let the Permissions screen decide which ones
 * actually stop.
 */
...
export function ApprovalProvider({ location, children }) {
  ...
  const finish = (person) => {
    ...
    if (person) {
      /* The log is the only record that anything happened here. With no crew
       * accounts and no shift sign-in, this IS the audit trail — who, what,
       * when, and which shop. Newest first, capped, same as the integration
...
function ApprovalDialog({ action, detail, roster, allows, namedCount, onCancel, onApprove }) {
  ...
  const submit = () => {
    ...
    if (!person) {
      /* Setting a PIN used to be offered right here, to whoever was standing
       * at the tablet — which meant the tablet could mint an approver. It
       * can't now: PINs are issued on the Team screen by an admin. So the
       * miss says where one comes from, since that is the only thing left to
       * do about it. */
    ...
    >
      <div className="space-y-4">
        {/* What is being approved, in the words of the thing that asked —
          *  never just "enter your PIN". Somebody handed a tablet across a
          *  bench deserves to read what they are putting their name to. */}
