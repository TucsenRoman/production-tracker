...
/** Stations are configured by the console, so a station is a NAME, not a
 *  fixed union. `STATIONS` below is only the seed. */
export type StationName = string;

export type StockState = "made" | "freezer" | "floor";

/** A severity, not an action — see `stockStatus`. */
...
export type StationKind = "workstation" | "process";

/** How a run missed its target. Two names, not a boolean — see `targetMiss`. */
export type TargetMiss = "slow" | "short";

/* Tone names are plain string unions rather than the UI's `BadgeTone` so the
 * domain never imports a component. They are subsets of it on purpose. */
export type MeterTone = "muted" | "ok" | "warn" | "danger";
export type PriorityTone = "neutral" | "info" | "warn" | "danger";

/** Where a product's weight is sitting. */
...
/** The band a product is managed to: reorder point, case capacity, and the
 *  smallest gap worth a changeover. */
...
export interface InventoryItem extends StockLevels, StockBand {
  product: string;
  unit: string;
  type: ProductFamily;
}

/** A record as it arrives from storage, Clover or the seed: older saves
 *  predate `made` and `type`, and the seed carries only `threshold` of the
 *  band. `normalizeItem` fills the gaps. */
export type StoredInventoryItem = Partial<InventoryItem> & { product: string };

/** A batch on the floor right now. */
export interface Batch {
  id: string;
  product: string;
  estWeight: number;
  boxWeight: number | null;
  /** A station name, or "Shelf-Ready" once it is through. */
  stage: string;
  needsSmoke: boolean;
  destination: StockState | null;
  startedAt: string;
  finalWeight?: number;
  /** Who last moved or closed it — shown on the card, not a permission. */
  lastActionBy?: string | null;
}

/** A finished batch. Feeds the yield and time comparisons. */
export interface ClosedBatch {
  ...
  /** Who closed it out. Absent on the seeded history. */
  closedBy?: string | null;
  /** Minutes spent, per station. **null when the floor closed it**: the
   *  tablet does not clock each stage, only the console's seeded history
   *  carries these. Readers must handle the null. */
  minutes: Record<StationName, number> | null;
}
...
export interface ScheduleEntry {
  ...
  qty: number;
  unit: string;
  /** null means planned and unstarted; an id means it is on the floor now. */
  batchId?: string | null;
}

/** date key → station → what is planned there. */
export type Schedule = Record<string, Record<StationName, ScheduleEntry[]>>;

/** One leg of a stocking route: how much to take, and from where. */
export interface PullPart {
  where: StockState;
  qty: number;
}
...
export interface Task {
  ...
  createdAt: string;
  completed: boolean;
  /** Cleared back to null when a task is un-completed, so not just optional. */
  completedBy?: string | null;
  completedAt?: string | null;

  /* Only on the stocking tasks the console generates from low stock. The
   * floor renders the route from `pullFrom` rather than parsing `note`, so it
   * survives an edit. A pull part carries no unit of its own — `unit` here is
   * what they are all counted in. */
  product?: string;
  qty?: number;
  unit?: string;
  pullFrom?: PullPart[];
...
/** Icons are stored as a string id, not a component: the list is persisted
 *  state, and a component reference does not survive JSON. */
...
export interface StationConfig {
  kind?: StationKind;
  /** A key into the station icon pool — see stations.tsx. */
  icon?: string;
  /** Legacy checkbox that meant exactly "process". */
  flagUnder?: boolean;
  overPct?: number | null;
...
export const STATIONS: StationName[] = ["Smokehouse", "Packaging"];

/** Target minutes in station, used to flag slow batches. */
export const STAGE_TARGET_MINUTES: Record<StationName, number> = {
  Smokehouse: 240,
  Packaging: 45,
};
...
export function dueLabel(key: string | null | undefined): string {
 ...
 */
export const STOCK_STATES: readonly StockStateMeta[] = [
...
const STATE_IDS: readonly StockState[] = STOCK_STATES.map((s) => s.id);

export const stateLabel = (id: string): string => STOCK_STATES.find((s) => s.id === id)?.label || id;

/** The short name is the PLACE, not the status: a stocking task says
 *  "40 lb freezer" (where to go), not "40 lb in stock". */
export const stateShort = (id: string): string => STOCK_STATES.find((s) => s.id === id)?.short || id;

/** Icon for each stock state — shared by every screen that shows one. */
export const STATE_ICON: Record<StockState, LucideIcon> = {
  made: PackageCheck,
  freezer: Snowflake,
  floor: Store,
};
 ...
 * the case into "Other".
 */
export const PRODUCT_TYPES: readonly ProductFamily[] = [
  "Bacon",
  "Brats",
  "Sausage",
  "Sticks",
 ...
 */
const FAMILY_RULES: readonly (readonly [ProductFamily, readonly string[]])[] = [
  ["Jerky", ["jerky"]],
...
export function productType(name = ""): ProductFamily {
 ...
 */
const FAMILY_THRESHOLD: Record<ProductFamily, number> = {
  Bacon: 40,
  Brats: 30,
  Sausage: 25,
  Sticks: 20,
...
export const defaultThreshold = (name: string): number => FAMILY_THRESHOLD[productType(name)] ?? DEFAULT_THRESHOLD;
 ...
 */
export const capacityFor = (threshold: number): number =>
  Math.max(threshold + 5, Math.round((threshold * 2.2) / 5) * 5);

/** What the stocker should bring out to fill the case. */
export const refillQty = (item: InventoryItem): number => Math.max(0, +(item.max - stockIn(item, "floor")).toFixed(1));
...
export const defaultMinBatch = (max: number): number =>
...
export const worthMaking = (item: InventoryItem, qty: number): boolean =>
  qty > 0 && qty >= (item.minBatch ?? defaultMinBatch(item.max));
export const worthMoving = (qty: number): boolean => qty > 0;

/** Older saved records predate `made` and `type`, so fill them in on read. */
export function normalizeItem(item: StoredInventoryItem): InventoryItem {
 ...
 * forever).
 */
export function setStockRange(
  item: StoredInventoryItem,
  { threshold, max, minBatch }: Partial<StockBand>,
): InventoryItem {
...
export const stockIn = (
  item: Partial<StockLevels> | null | undefined,
  state: StockState,
): number => Number(item?.[state] ?? 0);

export const totalStock = (item: Partial<StockLevels>): number =>
  +STATE_IDS.reduce((sum, id) => sum + stockIn(item, id), 0).toFixed(1);

const isLow = (item: InventoryItem): boolean =>
  stockIn(item, "floor") < item.threshold;

/** Stock standing behind the floor — what could be put out without making any. */
export const behindStock = (item: Partial<StockLevels>): number => +(stockIn(item, "made") + stockIn(item, "freezer")).toFixed(1);

/** How many lb short of the minimum the floor is. Zero when at or above par. */
export const floorDeficit = (item: InventoryItem): number =>
...
export function potentialCover(
  item: InventoryItem,
  perDay: number | null | undefined,
): number | null {
  if (!perDay || perDay <= 0) return null;
...
export const coverTone = (days: number | null | undefined): MeterTone =>
 ...
 */
export const canPutOut = (item: InventoryItem): boolean =>
...
export function moveStock(
  ...
  /* A patch object rather than computed keys in the spread: a computed key of
   * union type widens the literal to an index signature, which no longer
   * satisfies InventoryItem. */
  const patch: Partial<StockLevels> = {};
  patch[from] = +(available - qty).toFixed(1);
  patch[to] = +(stockIn(item, to) + qty).toFixed(1);
  return { ...item, ...patch };
}
 ...
 */
export function putOnFloor(item: InventoryItem): InventoryItem {
...
export const weighsInAt = (batch: Pick<Batch, "needsSmoke">): StationName | null => (batch.needsSmoke ? "Smokehouse" : null);
...
export function yieldTone(pct: number | null | undefined): MeterTone {
...
const STATION_KIND: Record<StationKind, StationKindSpec> = {
  workstation: {
    label: "Workstation",
...
const DEFAULT_STATION_KIND: StationKind = "workstation";

const kindOf = (config: StationConfig | undefined): StationKind => {
  if (config?.kind && STATION_KIND[config.kind]) return config.kind;
...
const toleranceFor = (config: StationConfig | undefined) => {
...
const targetMiss = (
  ...
  targets: Record<StationName, number> = {},
  configs: Record<StationName, StationConfig> = {},
): TargetMiss | null => {
...
export const TASK_CATEGORY_ICON_OPTIONS: { id: string; icon: LucideIcon }[] =
  Object.entries(TASK_CATEGORY_ICONS).map(([id, icon]) => ({ id, icon }));

export const iconFor = (iconId: string | undefined): LucideIcon =>
  (iconId ? TASK_CATEGORY_ICONS[iconId] : undefined) || ClipboardList;
...
export const categoryIcon = (
  ...
  id: string,
): LucideIcon => iconFor(categories.find((c) => c.id === id)?.iconId);

/** A category can't be removed while a task still points at it. */
export const categoryInUse = (tasks: readonly Task[], id: string): boolean => tasks.some((t) => t.category === id);
...
const priorityRank = (p: TaskPriority): number =>
  Math.max(0, TASK_PRIORITIES.indexOf(p));
